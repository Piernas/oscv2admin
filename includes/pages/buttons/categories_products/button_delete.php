<?php
  ...
  class button_delete {
    ...
    function get_value ($id) {
      ...
      $button ="";
      if ($id > 0) {
        $button = '                  <a id="product-delete-' . $product->products_id . '" href="javascript:ModalDeleteProduct(' . $product->products_id . ')"><i class="fas fa-trash fa-lg text-danger"></i></a>' . PHP_EOL;
      }
...
function ModalDeleteProduct(productsID){

  // Formulario
  $("form > .modal-content").unwrap();
    $(".modal-content").wrap('<form id="products_form" name="products" action="categories.php?pID=' + productsID + '&action=products_delete_confirm&cPath=$cPath" method="post">')
  ...
  var params = {"pID" : productsID, "action" : "products_delete"};
  $.ajax({
    data:  params,
    url:   '$url',
    type:  'get',
...
EOD;
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', '" . $this->cfg_key . 'SORT_ORDER' . "', '5', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Align', '" . $this->cfg_key . 'ALIGN' . "', 'Left', 'Alignment', '6', '1', 'tep_cfg_select_option(array(\'Left\', \'Right\', \'Center\'), ', now())");
    }
