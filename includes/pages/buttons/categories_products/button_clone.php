...
//        if ($id !=0) $this->value = $this->get_value($id);
      }
    }
  
    function get_value ($id) {
      global $product, $oscTemplate, $PHP_SELF, $cPath ;
      $button ="";
      if ($id > 0) {
        $button = '                  <a id="product-clone-' . $product->products_id . '" href="javascript:ModalCopyProduct(' . $product->products_id . ');"><i class="fas fa-clone fa-lg text-primary"></i></a>' . PHP_EOL;
      }
    ...
    function get_javascript () {
      ...
      $jScript = <<<EOD
 function ModalCopyProduct(productsID){
  // Formulario
  $("form > .modal-content").unwrap();
  $(".modal-content").wrap('<form id="copy_to" name="copy_to" action="categories.php?pID=' + productsID + '&action=products_copy_confirm&cPath=$cPath" method="post">')
  // Botones
  $("#ButtonCancelText").text("$message_cancel");
  $("#ModalButtonDelete").hide();
  $("#ModalButtonSave").hide();
  $("#ModalButtonMove").hide();
  $("#ModalButtonCopy").show();


    var params = {"pID" : productsID, "action" : "products_copy", "cPath" : "$cPath"};
...
EOD;
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', '" . $this->cfg_key . 'SORT_ORDER' . "', '3', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
