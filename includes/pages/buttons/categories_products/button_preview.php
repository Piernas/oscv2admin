  ...
  class button_preview {
    ...
    function get_value ($id) {
      ...
      if ($id > 0) {
        ...
        $preview_constant_query = tep_db_query ("SELECT user_password FROM administrators ORDER BY id DESC LIMIT 1");
        $preview_constant = tep_db_fetch_array ($preview_constant_query);
        $constant = hash ("md5", $preview_constant['user_password']);

        $button = '                <a href="' . tep_catalog_href_link ('product_info.php', "products_id=" . $product->products_id . "&admin_preview_id=" . $constant). '" target="_blank"><i class="fas fa-search-plus fa-lg text-info"></i></a>' . PHP_EOL;
      }
      return $button;
    }
