...
//        $this->value = $this->get_value($id);
    ...
    function get_value ($id) {
      ...
      if ($id > 0) {
        ...
        $button = '                  <a href="javascript:ModalMoveCategory(' . $category->categories_id . ');"><i class="fas fa-arrows-alt fa-lg text-primary"></i></a>' . PHP_EOL;
...
//        $button = '                  <a id="product-move-' . $product->products_id . '" href="javascript:ModalMoveProduct(' . $product->products_id . ');"><i class="fas fa-arrows-alt fa-lg text-primary"></i></a>' . PHP_EOL;
    ...
    function get_javascript () {
      ...
      $jScript = <<<EOD
function ModalMoveCategory (categoryID) {
  $("form > .modal-content").unwrap();
  $(".modal-content").wrap('<form id="products_form" name="categories" action="categories.php?categories_id=' + categoryID + '&action=categories_move_confirm&cPath=$cPath" method="post">')
  // Botones
  ...
  $("#ModalButtonCopy").hide();
    ...
    var params = {"cID" : categoryID, "action" : "categories_move", "cPath" : "$cPath"};
