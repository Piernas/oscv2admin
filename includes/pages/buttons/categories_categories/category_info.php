...
//        $this->value = $this->get_value($id);
    ...
    function get_value ($id) {
      ...
      if ($id > 0) {
        ...
        $button = '                  <a href="javascript:ModalInfoCategory(\'' . $category->categories_id . '\')"><i class="fas fa-info-circle fa-lg text-primary"></i></a>' . PHP_EOL;
    ...
    function get_javascript () {
      ...
      $jScript = <<<EOD
function ModalInfoCategory (categoryID){
  $("form > .modal-content").unwrap();
  $("#ButtonCancelText").text("$message_close");
  $("#ModalButtonDelete").hide();
  $("#ModalButtonSave").hide();
  $("#ModalButtonMove").hide();
  $("#ModalButtonCopy").hide();

  var params = {"categories_id" : categoryID, "action" : "categories_info"};
