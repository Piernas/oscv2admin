...
//        return '<a href="' . tep_href_link('categories.php', $category->get_path( $category->categories_id)) . '"><i class="fas fa-folder fa-lg text-primary"></i>&nbsp;<strong>' . $category->category_description[$languages_id]['categories_name'] . '</strong></a>';
        return $customer->customer_data['customers_email_address'];
      }
    }
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', '" . $this->cfg_key . 'SORT_ORDER' . "', '3', 'Sort order of display. Lowest is displayed last.', '6', '0', now())");
